            ...
            while True:
                ...
                if settings.noisy and not skipped:
                    ...
                    if mdl < settings.best_mdl:
                        ...
                        BEST_PROG.value = '\n'.join(format_rule(settings.order_rule(rule)) for rule in prog)
...
def popper(settings, best_prog):
    tester = Tester(settings)
    bkcons = get_bk_cons(settings, tester)
    Popper(settings, tester).run(bkcons, best_prog)
...
def learn_solution(settings):
    ...
    with multiprocessing.Manager() as manager:
        prog = manager.Value(c_char_p, "")
        p = multiprocessing.Process(target = popper, args = (settings, prog))
        p.start()
        ...
        return prog.value, terminated_by_timeout
