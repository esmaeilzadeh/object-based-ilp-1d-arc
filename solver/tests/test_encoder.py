...
def test_lean_block_atoms_typed():
    ...
    assert "block(0,b1,s1,v0)." in facts  # empty run is first-class
    assert "block(0,b2,s2,v5)." in facts
    assert "in_start(0,b0,s0)." in facts
    assert "in_rank(0,b0,r0)." in facts
    assert "in_rank(0,b2,r2)." in facts
    assert "rank_rev(r0,r2)." in facts
...
def test_object_bias_includes_cardinal_ordinal():
    ...
    text = render_object_bias_from_bk(lean, exs_text="pos(out_block(0,r0,s0,s3,v2)).")
...
def test_object_bias_head_only():
    ...
    assert "body_pred(size_lt,2)." in text
    assert "body_pred(cardinal_ordinal,2)." in text
    assert "type(out_block,('ex', 'rank', 'size', 'size', 'value'))." in text
    # No input-Bid anchoring constraint.
    assert "not body_literal(C, block, 4, (0,1,_,_))" not in text


def test_object_bias_size_add_bidirectional():
    """size_add legal if any arg is head Start/Len (compute or check)."""
    text = render_object_bias()
...
def test_encode_instance_object_only(tmp_path: Path):
    ...
    assert "block(" in bk
    assert "in_start(" in bk
    assert "in_rank(" in bk
    assert enc.bias_object_path is not None
    assert enc.bias_object_path.exists()
    exs = enc.exs_object_path.read_text()
    assert "out_block(" in exs
    # Independent out: rank/start form, not Bid/Off.
    assert "pos(out_block(0,r0,s0,s2,v1))." in exs
    ...
    assert "constant(r0, 'rank')." in bias


def test_mechanical_bias_from_bk_no_category():
    lean = "block(0,b0,s2,v1).\nobj_succ(0,b0,b2).\n"
    text = render_object_bias_from_bk(lean, exs_text="pos(out_block(0,r0,s0,s2,v1)).")
