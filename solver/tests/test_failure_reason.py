...
def test_object_b0_budgets():
    ...
    assert OBJECT_MAX_CLAUSES == 3
    assert OBJECT_MAX_LITERALS == 21
...
def test_literals_from_bias_matches_formula(tmp_path: Path):
    ...
    assert _literals_from_bias(bias) == 21
...
def test_solve_result_has_failure_fields():
    ...
    r = SolveResult(
        ...
        failure_detail={"max_literals": 21},
    )
    d = r.to_dict()
    assert d["failure_reason"] == "popper_timeout"
    assert d["failure_detail"]["max_literals"] == 21
