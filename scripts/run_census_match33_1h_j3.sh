#!/usr/bin/env bash
# 33 census-match (object-road) first-3 tasks @ 1h, 3-way parallel.
set -euo pipefail
...
DS_TMP="$(mktemp -d /tmp/census_match_first3_ds.XXXXXX)"
trap 'rm -rf "$DS_TMP"' EXIT

# Build a 33-task dataset tree (symlinks) where train census_match is true.
python - <<PY
import json
import os
...
for f in files:
    ...
    if not census_match(data["train"]):
        continue
